...
problem(problem(Name, Entity, Domain, R, OD, SI, DI, G, C, MS, LS))   
                %--> ['(',define,'(',problem,Name,')',
                --> ['(',define,'(',problem],typed_list(name, [Name|Entity]),[')',
                            '(',':',domain, Domain,')'],
                     (require_def(R)        ; []),
...
init(S,D)                         --> ['(',':',init], 
                                        ['(',':',static], zeroOrMore(init_el, S), [')'],
                                        ['(',':',dynamic],zeroOrMore(init_el, D), [')'],
                                        [')'].
