...
domainBNF(domain(N, R, T, O, P, F, C, S))
...
% Actions definitons
action_def(action(S, L, Parent, AbsArgs, KeepList, PosPrecon, NegPrecon, PosEqPrecon, NegEqPrecon, PosOrPrecon, NegOrPrecon, PosE, NegE, Assign))
                --> ['(',':',action], action_symbol(S),
                        [':',parameters,'('], typed_list(variable, L), [')'],
                        [':',parent,'('], name(Parent), ['('], zeroOrMore(variable, AbsArgs), [')',')'],
                        action_def_body(KeepList, PosPrecon, NegPrecon, PosEqPrecon, NegEqPrecon, PosOrPrecon, NegOrPrecon, PosE, NegE, Assign),
                    [')'].
action_symbol(N)        --> name(N).
action_def_body(Keep, PosP, NegP, PEqP, NEqP, PosOrP, NegOrP, PosE, NegE, Assign)
                --> (([':',static],       emptyOr(keep(Keep)))  ; []),
                    (([':',precondition], emptyOr(precon(PosP, NegP, PEqP, NEqP, PosOrP, NegOrP)))  ; []),
                    (([':',effect],       emptyOr(effect(PosE, NegE, Assign)))  ; []).
...
precon(P, N, PEq, NEq, OrP, OrN)          --> ['(',and], c_precon(P, N, PEq, NEq, OrP, OrN), [')'].
precon(P, N, PEq, NEq, OrP, OrN)          --> c_precon(P, N, PEq, NEq, OrP, OrN).
c_precon(P, N, PEq, NEq, OrP, OrN)        --> p_precon(P, N, PEq, NEq, OrP, OrN).
p_precon([], [], [], [], [], [])        --> [].
p_precon(P, N, PEq, NEq, OrP, OrN)      --> ['(',or], or_precon(P, N, PEq, NEq, OrP, OrN), [')'].
p_precon(P, N, PEq, [F|NEq], OrP, OrN)  --> ['(',not], equal_term(F), !, [')'], p_precon(P, N, PEq, NEq, OrP, OrN).
p_precon(P, [F|N], PEq, NEq, OrP, OrN)  --> ['(',not], atomic_formula(term,F), [')'], p_precon(P, N, PEq, NEq, OrP, OrN).
p_precon(P, N, [F|PEq], NEq, OrP, OrN)  --> equal_term(F), !, p_precon(P, N, PEq, NEq, OrP, OrN).
p_precon([F|P], N, PEq, NEq, OrP, OrN)  --> atomic_formula(term, F), p_precon(P, N, PEq, NEq, OrP, OrN).

or_precon([], [], [], [], [], [])       --> [].
or_precon(P, N, PEq, NEq, OrP, [F|Ns])    --> ['(',not], atomic_formula(term,F), [')'], or_precon(P, N, PEq, NEq, OrP, Ns).
or_precon(P, N, PEq, NEq, [F|Ps], OrN)    --> atomic_formula(term, F), or_precon(P, N, PEq, NEq, Ps, OrN).

effect(P, N, A)                 --> ['(',and], c_effect(P, N, A), [')'].
effect(P, N, A)                 --> c_effect(P, N, A).
%c_effect(forall(E))            --> ['(',forall,'('], typed_list(variable)∗) effect(E), ')'.    %:conditional-effects
...
c_keep(K)               --> p_keep(K).
p_keep([])              --> [].
p_keep([F|Ps])          --> atomic_formula(term, F), p_keep(Ps).

equal_term(F)           --> ['(','='], zeroOrMore(term, T), [')'], {F =.. ['='|T]}.
